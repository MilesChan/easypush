...
void write_pidfile(){
	...
	if(strlen(pidfile)){
		...
		_snprintf(buf, sizeof(buf), "%d", pid);
...
#else
	HANDLE g_hMutex = CreateMutexA(NULL,TRUE,"icomet-master");
	if(ERROR_ALREADY_EXISTS == GetLastError()) {
		fprintf(stderr, "Fatal error!\ncomet-server is already running!\n"
			"You must kill the process before starting icomet.\n");
		exit(0);
	}
#endif
}
